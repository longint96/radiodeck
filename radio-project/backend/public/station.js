...
const state = {
  ...
  mode: null,
  globalPort: null,
  domain: null, // если задан на портале — используется вместо window.location.hostname
};

/**
 * Хост для ссылки на поток — тот же принцип, что и на портале.
 */
...
function updateListenUrl() {
  ...
  const url = `http://${getStreamHost()}:${state.globalPort}/${mount}`;
