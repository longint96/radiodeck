const state = {
  ...
  globalPort: null,
  domain: null, // если задан на сервере — используется вместо window.location.hostname в ссылках на поток
...
/**
 * Хост для ссылок на поток (карточки станций, плейлист .m3u, и т.д.) —
 * заданный в ENGINE SETTINGS домен, если он есть, иначе тот адрес,
 * по которому сейчас реально открыт портал (мог быть IP или домен —
 * что угодно оказалось в адресной строке браузера).
 */
function getStreamHost() {
  return state.domain || window.location.hostname;
}
...
renderThemePicker('settingsThemePicker');
...
async function loadGlobal() {
  try {
    ...
    document.getElementById('globalDomainInput').value = g.domain || '';
...
async function loadStations() {
  ...
  try {
    ...
    grid.innerHTML = data.stations.map((s) => {
      const streamUrl = `http://${getStreamHost()}:${state.globalPort || '8000'}/${s.mount}`;
      return `
...
document.getElementById('downloadPlaylistBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    data.stations.forEach((s) => {
      const url = `http://${getStreamHost()}:${port}/${s.mount}`;
