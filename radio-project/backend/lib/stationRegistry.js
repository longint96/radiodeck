...
async function updateGlobalSettings({ port, sourcePassword, domain }) {
  if (port !== undefined && (!Number.isInteger(port) || port < 1 || port > 65535)) {
    throw new Error('Некорректный порт (1-65535)');
  }
  if (domain !== undefined && domain !== null && domain !== '') {
    const cleanDomain = String(domain).trim();
    if (/\s/.test(cleanDomain) || cleanDomain.includes('/')) {
      throw new Error('Домен не должен содержать пробелы или "/" — только сам домен/IP, без пути и протокола');
    }
    if (cleanDomain.length > 253) {
      throw new Error('Слишком длинное значение домена');
    }
  }
  ...
  return withWriteLock(() => {
    ...
    if (domain !== undefined) {
      // Пустая строка/null — значит "не задан", вернёмся к автоопределению
      // по window.location.hostname на фронтенде
      const cleanDomain = domain === null ? '' : String(domain).trim();
      registry.global.domain = cleanDomain || null;
    }
    writeRegistryRaw(registry);
...
function exportBackup() {
  ...
  return {
    ...
    global: {
      ...
      domain: registry.global.domain || null,
    },
...
async function importBackup(backup) {
  ...
  const { port, sourcePassword, mediaBaseDir, domain } = backup.global;
  ...
  // domain — необязательное поле (бэкапы, сделанные до появления этой
  // настройки, его не содержат вовсе — не ошибка, просто "не задан")
  let cleanDomain = null;
  if (domain !== undefined && domain !== null && domain !== '') {
    if (typeof domain !== 'string' || /\s/.test(domain) || domain.includes('/')) {
      throw new Error('Некорректное значение domain в разделе global бэкапа');
    }
  ...
  return withWriteLock(() => {
    const newRegistry = {
      global: { port, sourcePassword, mediaBaseDir, domain: cleanDomain },
